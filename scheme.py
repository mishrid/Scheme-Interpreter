...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        if scheme_length(vals) != scheme_length(formals):
            raise SchemeError
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda operand_expression: scheme_eval(operand_expression,env)), env)
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 22
        return scheme_apply(self, operands.map(lambda operand_expression: operand_expression), env)
        # END Problem 22
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        procedure = expressions.second.first
        value = scheme_eval(procedure,env)
...
def do_and_form(expressions, env):
    ...
    evaluation_of_expression =  scheme_eval(expressions.first, env)  

    if scheme_truep(expressions) == True and expressions.second == nil: 
        return evaluation_of_expression

    elif scheme_truep(evaluation_of_expression) == False: 
        return scheme_truep(evaluation_of_expression)
...
def do_or_form(expressions, env):
    ...
    if scheme_truep(evaluation_of_expression) == True: 
        return evaluation_of_expression
...
def make_let_frame(bindings, env):
    ...
    def formals(bindings):
        ...
        formal1 = bindings.first.first
        formal2 = bindings.second

        if formal2 == nil and formal1 != nil:
            return Pair(formal1, nil)
        else:
            return Pair(formal1, formals(formal2))

    def values(bindings):
        check_form(bindings.first, 2)

        value1 = bindings.first.second.first
        value_nil = bindings.first.second.second

        if value_nil != nil:
            raise SchemeError

        if type(value1) != int:
            value1 = scheme_eval(value1, env)

        value2 = bindings.second

        if value2 == nil and value1 != nil:
            return Pair(value1, value2)
        else:
            return Pair(value1, values(value2))

    formal_pairs = formals(bindings)
    value_pairs = values(bindings)
    check_formals(formal_pairs)

    return env.make_child_frame(formal_pairs, value_pairs)
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    return MuProcedure(formals,values)
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            arguments = rest.map(lambda operands: scheme_eval(operands,env,True))
